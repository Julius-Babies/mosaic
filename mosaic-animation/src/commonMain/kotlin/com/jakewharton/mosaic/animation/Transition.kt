 ...
 * [Transition.animateFloat], [animateColor][animateColor],
 ...
 * [animateColor][animateColor] etc. When the [targetState] changes,
...
internal constructor(
	...
	internal var playTimeNanos: Long
		get() {
	...
	internal val hasInitialValueAnimations: Boolean
		get() =
	...
	private fun resetAnimations() {
		...
		_transitions.forEach { it.resetAnimations() }
	}
	...
	/**
	 * Each animation created using [animateFloat], etc is represented as a
	...
	internal constructor(
		...
		init {
			val visibilityThreshold: T? =
				VisibilityThresholdMap[typeConverter]?.let {
	...
	private class SegmentImpl<S>(override val initialState: S, override val targetState: S) : Segment<S> {
...
public inline fun <S> Transition<S>.animateColor(
	...
	return animateValue(Color.VectorConverter, transitionSpec, label, targetValueByState)
}
...
public fun InfiniteTransition.animateColor(
	...
	return animateValue(initialValue, targetValue, Color.VectorConverter, animationSpec, label)
}
