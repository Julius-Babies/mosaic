...
import com.jakewharton.mosaic.animation.ArcMode.Companion.ArcBelow
import com.jakewharton.mosaic.animation.ArcMode.Companion.ArcLinear
import com.jakewharton.mosaic.animation.KeyframesSpec.KeyframesSpecConfig
import kotlin.math.abs
import kotlin.math.roundToInt

public object AnimationConstants {
	/** The default duration used in [com.jakewharton.mosaic.animation.VectorizedAnimationSpec]s and [AnimationSpec]. */
 ...
 * out of the box. For example, to animate [IntOffset] the system uses
...
public interface AnimationSpec<T> {
	 ...
	 * [AnimationVector] to animate. [com.jakewharton.mosaic.animation.VectorizedAnimationSpec] describes how the converted
 ...
 *
 * @param durationMillis duration of the [com.jakewharton.mosaic.animation.VectorizedTweenSpec] animation.
 * @param delay the number of milliseconds the animation waits before starting, 0 by default.
 * @param easing the easing curve used by the animation. [com.jakewharton.mosaic.animation.FastOutSlowInEasing] by default.
 */
@Immutable
public class TweenSpec<T>(
	public val durationMillis: Int = AnimationConstants.DefaultDurationMillis,
 ...
 *
 * @param dampingRatio damping ratio of the spring. [com.jakewharton.mosaic.animation.Spring.DampingRatioNoBouncy] by default.
 * @param stiffness stiffness of the spring. [com.jakewharton.mosaic.animation.Spring.StiffnessMedium] by default.
 ...
 * @param durationMillis Duration of the animation. [AnimationConstants.DefaultDurationMillis] by default.
...
public sealed class KeyframesSpecBaseConfig<T, E : KeyframeBaseEntity<T>> {
	...
	public var durationMillis: Int = AnimationConstants.DefaultDurationMillis
	 ...
	 *   minimum value of `0`.
	 * @return an instance of [E] so a custom [com.jakewharton.mosaic.animation.Easing] can be added by the [using] method.
	 */
	...
	/**
	 * Adds an [com.jakewharton.mosaic.animation.Easing] for the interval started with the just provided timestamp. For example: 0f
	 * at 50 using LinearEasing
	 *
	 * @param easing [com.jakewharton.mosaic.animation.Easing] to be used for the next interval.
 ...
 *
 * [com.jakewharton.mosaic.animation.VectorizedKeyframesSpec] animates based on the values defined at different timestamps in the
 * duration of the animation (i.e. different keyframes). Each keyframe can be defined using
 * [KeyframesSpecConfig.at]. [com.jakewharton.mosaic.animation.VectorizedKeyframesSpec] allows very specific animation definitions
 * with a precision to millisecond.
 *
 * For each interval, you may provide a custom [com.jakewharton.mosaic.animation.Easing] by use of the [KeyframesSpecConfig.using]
 * function.
 *
 ...
 * [KeyframesSpecConfig.using] and [com.jakewharton.mosaic.animation.ArcMode]:
 *
 * If instead, you wish to have a smooth curvy animation across all intervals, consider using
 * [KeyframesWithSplineSpec].
 */
@Immutable
public class KeyframesSpec<T>(public val config: KeyframesSpecConfig<T>) : DurationBasedAnimationSpec<T> {
	/**
	...
	public class KeyframesSpecConfig<T> : KeyframesSpecBaseConfig<T, KeyframeEntity<T>>() {
		 ...
		 * @param timeStamp The time in the during when animation should reach value: [this], with a
		 *   minimum value of `0`.
		 * @return an [KeyframeEntity] so a custom [com.jakewharton.mosaic.animation.Easing] can be added by [with] method.
		 */
		 ...
		 *
		 * Note that arc modes are meant for objects with even dimensions (such as [IntOffset] and
		 * its variants). Where each value pair is animated as an arc. So, if the object has odd
		 ...
		 * vector converter in [AnimationSpec.vectorize].
		 */
	...
	internal constructor(
		...
		internal var arcMode: ArcMode = ArcLinear,
 ...
 * it most to animate positional 2D values such as [IntOffset]. For example:
 *
...
public class KeyframesWithSplineSpec<T>(
	 ...
	 * [com.jakewharton.mosaic.animation.ArcMode] between intervals (compared to [KeyframesSpecConfig] used for [keyframes]).
	 */
 ...
 * [com.jakewharton.mosaic.animation.ArcMode]:
 *
